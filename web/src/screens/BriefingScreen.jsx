 ...
 *
 * Every string on the timeline and in the decisions comes from the domain,
 * already in the reader's language: this screen renders the cycle's own words
 * (packages/core/src/briefing/nightlyCycle.js) rather than paraphrasing them.
 */
  ...
  const approve = async (decision) => {
    try {
      ...
      setReceipts((previous) => ({
        ...previous,
        [decision.id]: t('common.ticketCreatedWithDue', {
          id: ticket.id,
          owner: ticket.owner,
      ...
      setReceipts((previous) => ({
        ...previous,
        [decision.id]: errorText(failure, 'common.ticketFailed'),
      }));
    }
  };

  const decisionsByNode = data ? groupDecisionsByNode(data.timeline, data.decisions) : new Map();
  ...
  return (
    <>
      <DataPanel
        ...
        kicker={
          data
            ? t('briefing.kicker', { date: fmt.fullDate(data.generatedAt) })
            : t('briefing.kickerPlain')
        }
          ...
          data ? (
            <span className="panel-meta">
              {t('briefing.meta', {
                ...
                reviews: fmt.integer(data.reviewsRead),
                cost: fmt.integer(data.costIdr),
              })}
            </span>
          ) : null
        }
        loading={{ message: t('briefing.loading') }}
        empty={{
          title: t('briefing.emptyTitle'),
          description: t('briefing.emptyDescription'),
          onAction: reload,
        }}
        error={{
          title: t('briefing.errorTitle'),
          description: errorText(error, 'briefing.errorFallback'),
          onRetry: reload,
        }}
      >
        {data ? (
 ...
 *
 * Matched on `agentKey`, which the cycle states, rather than on the agent's
 * display name — that name is now two different strings depending on who is
 * reading, and a lookup keyed by it would find nothing in English.
 */
function groupDecisionsByNode(timeline, decisions) {
  ...
  for (const decision of [...decisions].sort((a, b) => a.rank - b.rank)) {
    const index = lastIndexOfAgent.get(decision.agentKey) ?? fallback;
  ...
  return (
    <div className="decision">
      ...
      <div className="decision-head">
        <span className="tag tag-outline">{t('briefing.decisionTag', { rank: decision.rank })}</span>
        <span className="decision-meta">
          {t('briefing.decisionMeta', { time: decision.time, agent: decision.agent })}
        </span>
      </div>
      ...
      {!canAct ? <p className="state-note">{t('common.readOnlyApproveDecision')}</p> : null}
    </div>
  );
}
